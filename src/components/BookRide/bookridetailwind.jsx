...
const BookRide = () => {
  ...
  const f = () => {
    axios.get('https://localhost:8082/app/bookride/getallrides').then((res) => {
  ...
  const handleBookRide = () => {
    if (logindata.email === '') {
      ...
      axios
        .put(
          `https://localhost:8082/app/bookride/updateRideCompletionStatus/${selectedRideData.id}`
        )
        .then()
        .catch((err) => {
          console.log(err)
      ...
      axios
        .post(
          `https://localhost:8082/app/userRideHistory/${logindata.email}/${selectedRideData.id}`
        )
        .then((response) => {
  ...
  const handleFilterChanges = (e) => {
    ...
    try {
      if (leavingFromFilters.length == 0 && goingToFilters.length == 0) {
        ...
        const res = axios.post('https://localhost:8082/app/bookride/filter', [
          leavingFromFilters,
          goingToFilters,
        ])
        res.then((res) => {
  ...
  useEffect(() => {
    ...
    try {
      const response = axios.get(
        `https://localhost:8082/app/bookride/selectedValue/${selectedid.idSelected}`
      )
      response.then((res) => {
      ...
      <div className="sortingContainer">
        <div className="pt-1 pb-1 pl-1 flex gap-3 m-3">
          ...
          <button className="py-2 px-4 rounded-full text-xs bg-white border border-lightgray flex items-center transition-all hover:pl-3 hover:pr-3 hover:pt-2">
            <Typography onClick={() => setOpendilog(!opendilog)}>
              Filter
            </Typography>
              ...
              <div className="flex flex-col w-3/5">
                ...
                <div className="mt-4">
                  <div className="flex justify-between">
                    ...
                    <Typography className="py-2 text-4xl">
                ...
                <div className="pt-4">
                  ...
                  <div className="pb-4 ">
                    <h1 className="pb-4 pt-5 text-center text-3xl">
                      Driver Profile
                    </h1>
                    <div className="flex justify-between items-center">
